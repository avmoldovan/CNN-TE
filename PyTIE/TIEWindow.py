    ...
    def computeAverageLocalOfObservations(self):
        ...
        for pastVal in range(0, self.base_power_k):
            ...
            for destVal in range(0, self.base):
                ...
                p_in = float(self.pastCount[pastVal]) / float(self.observations)
                p_in_in1 = float(self.nextPastCount[destVal][pastVal])  / float(self.observations)
                for sourceVal in range(0, self.base_power_l):
                    if self.sourceNextPastCount[sourceVal][destVal][pastVal] != 0:
                        ...
                        # this is p(i_n+1,i_n,j_n)
                        p_in_in1_jn = float(self.sourceNextPastCount[sourceVal][destVal][pastVal]) / float(self.observations)

                        p_in_jn = float(self.sourcePastCount[sourceVal][pastVal]) / float(self.observations)



                        localValue = p_in_in1_jn / p_in_jn - p_in_in1 / p_in
                        teCont = p_in_in1_jn * localValue
